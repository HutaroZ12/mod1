...
class LoadingState extends MusicBeatState
{
	...
	function onLoad()
	{
		...
		if(!gotError) MusicBeatState.switchState(target);
		transitioning = true;
	...
	public static function prepareToSong()
	{
		...
		.then((_) -> new Future<Bool>(() -> {
			...
			else songsToPrepare.push('$folder/Inst');
	...
	static function clearInvalidFrom(arr:Array<String>, prefix:String, ext:String, type:AssetType, ?parentFolder:String = null)
	{
		for (folder in arr.copy())
		{
			...
			if(nam.endsWith('/'))
			{
				...
				#if debug trace('Folder detected! ' + folder); #end
			}
		}
		...
		while(i < arr.length)
		{
			...
			#if debug trace('attempting on $prefix: $myKey'); #end
			
			if(member.endsWith('/') || (!Paths.fileExists(myKey, type, false, parentFolder)))
			{
				arr.remove(member);
				#if debug trace('Removed invalid $prefix: $member'); #end
			}
			else i++;
		}
	}
		...
		try
		{
			...
			else
			{
				for (i in 0...10)
				{
					...
					{
						#if debug trace('found Sprite PNG'); #end
	...
	static function preloadSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true):Null<Sound>
	{
		...
		#if debug trace('precaching sound: $file'); #end
