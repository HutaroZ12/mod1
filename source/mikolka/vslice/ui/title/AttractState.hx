...
class AttractState extends MusicBeatSubstate
{
	...
	public override function create():Void
	{
		...
		#if html5
		#if debug trace('Playing web video ${ATTRACT_VIDEO_PATH}'); #end
		playVideoHTML5(ATTRACT_VIDEO_PATH);
		#end

		#if (hxvlc || hxCodec)
		if (ATTRACT_VIDEO_PATH == '')
			ATTRACT_VIDEO_PATH = ModsHelper.collectVideos();
		#if debug trace('Playing native video ${ATTRACT_VIDEO_PATH}'); #end
	...
	public override function update(elapsed:Float):Void
	{
		...
		#else
		if (TouchUtil.justPressed || FlxG.keys.justPressed.ANY
			&& !controls.justPressed("volume_up")
			&& !controls.justPressed("volume_down")
			&& !controls.justPressed("volume_mute"))
		#end
		{
			onAttractEnd();
		}
	...
	function onAttractEnd():Void
	{
		...
		{
			FlxG.sound.playMusic(Paths.music('freakyMenu'), 0.7 * ClientPrefs.data.bgmVolume);
