...
class FreeplayState extends MusicBeatSubstate
{
	...
	override function create():Void
	{
		...
		var allSongs = FreeplayHelpers.loadSongs();
		for (index => sngCard in allSongs)
		{
		...
		topLeftCornerText.font = OldPaths.font("vcr.ttf");
		topLeftCornerText.visible = false;
		topLeftCornerText.antialiasing = ClientPrefs.data.antialiasing;
		...
		ostName.font = OldPaths.font("vcr.ttf");
		...
		letterSort.changeSelectionCallback = (str) ->
		{
			...
			{
				FunkinSound.playOnce(Paths.sound('scrollMenu'), 0.4 * ClientPrefs.data.sfxVolume);
				curSelected = 1;
		...
		missingText.setFormat(OldPaths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	var randomCapsule:SongMenuItem;
	var tempSong:Null<FreeplaySongData>;
	var funnyMenu:SongMenuItem;
	...
	function rankDisplayNew(fromResults:Null<FromResultsParams>):Void
	{
		...
		new FlxTimer().start(0.1, _ ->
		{
			...
			{
				case SHIT:
					FunkinSound.playOnce(Paths.sound('ranks/rankinbad'), ClientPrefs.data.sfxVolume);
				case PERFECT:
					FunkinSound.playOnce(Paths.sound('ranks/rankinperfect'), ClientPrefs.data.sfxVolume);
				case PERFECT_GOLD:
					FunkinSound.playOnce(Paths.sound('ranks/rankinperfect'), ClientPrefs.data.sfxVolume);
				default:
					FunkinSound.playOnce(Paths.sound('ranks/rankinnormal'), ClientPrefs.data.sfxVolume);
			}
			rankCamera.zoom = 1.3;
	...
	function rankAnimSlam(fromResultsParams:Null<FromResultsParams>)
	{
		...
		{
			case SHIT:
				FunkinSound.playOnce(Paths.sound('ranks/loss'), ClientPrefs.data.sfxVolume);
			case GOOD:
				FunkinSound.playOnce(Paths.sound('ranks/good'), ClientPrefs.data.sfxVolume);
			case GREAT:
				FunkinSound.playOnce(Paths.sound('ranks/great'), ClientPrefs.data.sfxVolume);
			case EXCELLENT:
				FunkinSound.playOnce(Paths.sound('ranks/excellent'), ClientPrefs.data.sfxVolume);
			case PERFECT:
				FunkinSound.playOnce(Paths.sound('ranks/perfect'), ClientPrefs.data.sfxVolume);
			case PERFECT_GOLD:
				FunkinSound.playOnce(Paths.sound('ranks/perfect'), ClientPrefs.data.sfxVolume);
			default:
				FunkinSound.playOnce(Paths.sound('ranks/loss'), ClientPrefs.data.sfxVolume);
	...
	function changeDiff(change:Int = 0, forceUpdateSongList:Bool = false):Void
	{
		...
		if (daSong != null)
		{
			...
			var songScore:Float = Highscore.getScore(daSong.getNativeSongId(), diffId);
			// Save.instance.getSongScore(grpCapsules.members[curSelected].songData.songId, suffixedDifficulty);
	...
	function capsuleOnConfirmDefault(cap:SongMenuItem, ?targetInstId:String):Void
	{
		...
		var songLowercase:String = OldPaths.formatToSongPath(targetSong.getNativeSongId());
	...
	function changeSelection(change:Int = 0, updateCardPosition:Bool = true):Void
	{
		...
		} else if (curSelected >= grpCapsules.countLiving()) {
			if (updateCardPosition)
			{
				curSelected = 0;
				change = 0;
